/* global React */
const { useState: useJState, useEffect: useJEffect, useMemo: useJMemo } = React;

function Journey() {
  const [data, setData] = useJState(null);
  const [err, setErr] = useJState(null);
  const [sortKey, setSortKey] = useJState('applied');
  const [sortDir, setSortDir] = useJState('desc');
  const [statusFilter, setStatusFilter] = useJState('all');
