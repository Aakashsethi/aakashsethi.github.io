...
module Auth
  ...
  def self.send_link(email, token)
    ...
    esc_link = link.gsub('&', '&amp;').gsub('"', '&quot;')
    html = <<~HTML
      <!doctype html>
      <html><body style="font-family:-apple-system,Segoe UI,Helvetica,Arial,sans-serif;color:#1c1917;line-height:1.5;padding:24px;">
        <p>Sign in to Aakash's demos.</p>
        <p><a href="#{esc_link}" style="display:inline-block;padding:10px 16px;background:#1c1917;color:#fafaf9;border-radius:6px;text-decoration:none;">Sign in →</a></p>
        <p style="color:#57534d;font-size:13px;">Or paste this link into your browser (expires in 15 minutes):<br><a href="#{esc_link}">#{esc_link}</a></p>
        <p style="color:#78716c;font-size:12px;">Didn't request this? You can ignore this email — the link expires shortly and no account was created.</p>
      </body></html>
    HTML
  ...
  def self.consume(token:, ip:, ua:)
    ...
    # Create session. The cookie holds the raw secret; the DB stores only its
    # SHA256 so a DB dump can't be replayed as live sessions.
    session_id = SecureRandom.hex(32)
    Store.conn.exec_params(<<~SQL, [session_token_hash(session_id), user_id, now, now, now + SESSION_TTL, Limiter.hash(ip), Limiter.hash(ua)])
  ...
  def self.current_user(session_id)
    ...
    now = Time.now.to_i
    sid_hash = session_token_hash(session_id)
    row = Store.conn.exec_params(<<~SQL, [sid_hash, now]).first
    ...
    Store.conn.exec_params('UPDATE sessions SET last_seen_at = $1 WHERE id = $2', [now, sid_hash])
    { id: row['user_id'].to_i, email: row['email'] }
  end

  def self.revoke(session_id)
    return if session_id.nil? || session_id.empty?
    Store.conn.exec_params('UPDATE sessions SET revoked_at = $1 WHERE id = $2', [Time.now.to_i, session_token_hash(session_id)])
  end

  def self.session_token_hash(session_id)
    Digest::SHA256.hexdigest("sess:v1:#{session_id}")
  end
